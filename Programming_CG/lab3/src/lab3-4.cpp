...
const char *ShaderProgram::VSFile = "../lab3-4_vs.glsl";
const char *ShaderProgram::FSFile = "../lab3-4_fs.glsl";
const char *ShaderProgram::TCSFile = "../lab3-4_tc.glsl";
const char *ShaderProgram::TESFile = "../lab3-4_te.glsl";
...
void ReloadShaders()
{
    glDetachShader(SP->program, SP->vertexShader);
    glDetachShader(SP->program, SP->fragmentShader);
    glDetachShader(SP->program, tcs);
    glDetachShader(SP->program, tes);
    SP->AddVertexShader();
    SP->AddFragmentShader();
    tcs = SP->CreateShader(SP->TCSFile, GL_TESS_CONTROL_SHADER);
    SP->AddShader(tcs, "TCS");
    tes = SP->CreateShader(SP->TESFile, GL_TESS_EVALUATION_SHADER);
    SP->AddShader(tes, "TES");
    SP->Use();
...
int main(int argc, char const *argv[])
{
    ...
    GLuint textureMap, environmentMap, bumpMap, displacementMap;
    ...
    std::cout << "------ Displacement mapping ------\n";
    create2DTexture(3, displacementMap, "../../common/data/numberline_df_inv_gaussblur.png");
    ...
    std::cout << "Drawing " << num_faces << " patches" << '\n';
    render(window);
...
void render(GLFWwindow *window)
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Unifoms
        GLint uniLoc = glGetUniformLocation(SP->program, "modelMatrix");
        glUniformMatrix4fv(uniLoc, 1, GL_FALSE, glm::value_ptr(modelMatrix));

        uniLoc = glGetUniformLocation(SP->program, "modelViewMatrix");
        glUniformMatrix4fv(uniLoc, 1, GL_FALSE, glm::value_ptr(modelViewMatrix));

        uniLoc = glGetUniformLocation(SP->program, "modelViewProjectionMatrix");
        glUniformMatrix4fv(uniLoc, 1, GL_FALSE, glm::value_ptr(MVP));

        uniLoc = glGetUniformLocation(SP->program, "normalMatrix");
        ...
        /*  glUniform1i(sampler_unit, value)
        ...
        */
        ...
        texLoc = glGetUniformLocation(SP->program, "heightmap");
        glUniform1i(texLoc, 3); /* send texture in position 2 in texture array */
