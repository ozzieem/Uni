...
void Camera3D::getViewMatrix(Matrix4& mat)
{
	Matrix4 mt, mr;
	mt.setTranslation(-this->position.x,-this->position.y,-this->position.z);
	mr.genTransformMat(this->pitch,this->yaw,0);
...
void Camera3D::setRotationSpeed(float xAxis, float yAxis)
{
    this->rotSpeedX = xAxis;
    this->rotSpeedY = yAxis;
}
void Camera3D::setTranslationSpeed(float speed)
{
    this->tranSpeed = speed;
}
