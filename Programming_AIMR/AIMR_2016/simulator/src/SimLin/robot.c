...
struct Robot *CreateRobot()
{
  ...
  robot->State      = (u_char)0;
	...
	for(i=0;i<2;i++) {
  	...
  	robot->Motor[i].Max = 20;  //--KL tune these for position controller
  	robot->Motor[i].Acc = 64;  //--KL
		robot->Motor[i].Pos = 0;
		robot->Motor[i].TargetPos = 0;
		robot->Motor[i].mode = SPEED_MODE;
	}

  robot->IRSensor[5].X     = 0.0;
  robot->IRSensor[5].Y     = 25.5;
  robot->IRSensor[5].Alpha = M_PI/2.0;
  robot->IRSensor[5].DistanceValue = 0;
  robot->IRSensor[5].LightValue    = 500;

  robot->IRSensor[6].X     = 18.0;
  robot->IRSensor[6].Y     = 17.5;
  robot->IRSensor[6].Alpha = M_PI/4.0;
  robot->IRSensor[6].DistanceValue = 0;
  robot->IRSensor[6].LightValue    = 500;

  robot->IRSensor[7].X     = 24.5;
  robot->IRSensor[7].Y     = 5.5;
  robot->IRSensor[7].Alpha = 0.0;
  robot->IRSensor[7].DistanceValue = 0;
  robot->IRSensor[7].LightValue    = 500;

  robot->IRSensor[0].X     = 24.5;
  robot->IRSensor[0].Y     = -5.5;
  robot->IRSensor[0].Alpha = 0.0;
  ...
  robot->IRSensor[2].X     = 0.0;
  robot->IRSensor[2].Y     = -25.5;
  robot->IRSensor[2].Alpha = -M_PI/2.0;
  robot->IRSensor[2].DistanceValue = 0;
  robot->IRSensor[2].LightValue    = 500;

  robot->IRSensor[3].X     = -23.0;
  robot->IRSensor[3].Y     = -9.5;
  robot->IRSensor[3].Alpha = -2.5;
  robot->IRSensor[3].DistanceValue = 0;
  robot->IRSensor[3].LightValue    = 500;

  robot->IRSensor[4].X     = -23.0;
  robot->IRSensor[4].Y     = 9.5;
  robot->IRSensor[4].Alpha = 2.5;
...
void SolveEffectors(struct Context *context) {
  struct Robot *robot;
  double delta_direction,noise1,noise2,noise3,delta_speed,dpos,dpos_decel,speed_increment;
	short i, accel;
  ...
  //This part sets the target speed
	if(robot->Motor[0].mode == POSITION_MODE)
	{
	  //For each wheel
		for(i=0;i<2;i++)
		{
			...
			//If we're there, aim to stop
			if(robot->Motor[i].Pos == robot->Motor[i].TargetPos)
				robot->Motor[i].TargetValue = 0;
			else
			{
			  //--KL added explicit cast
			  //Delta pos at which we should start to decelerate
				dpos_decel = abs((int)(0.5 * pow(robot->Motor[i].Value,2.0) / ((double)accel/256.0)));

        //Delta pos
				dpos = (double)(robot->Motor[i].TargetPos - robot->Motor[i].Pos);

        //--KL added explicit cast
				if(abs((int)dpos) >= dpos_decel)
				{
					// do not decelerate yet
					robot->Motor[i].TargetValue = robot->Motor[i].Max;

					if(dpos < 0.0)
					  robot->Motor[i].TargetValue *= -1;
				}
				else
					// decelerate
					robot->Motor[i].TargetValue = 0;
  ...
  NormRad(robot->Alpha);
	...
	//Set new values for the speed and for the encoder positions
	for(i=0;i<2;i++)
	{
	  //Set new speed values
		...
		//--KL use increment
		speed_increment = (double)(accel)*ENCODER_FACTOR/256.0;

		//--KL Force no acceleration if using speed mode
		if( (abs((int)delta_speed) < speed_increment) ||
		    (robot->Motor[0].mode == SPEED_MODE))
			robot->Motor[i].Value = (double) robot->Motor[i].TargetValue;
		else
			if(delta_speed > 0.0)
			  robot->Motor[i].Value += speed_increment; //--KL
			else
			  robot->Motor[i].Value -= speed_increment; //--KL

printf("%lf  %lf  %lf \n",robot->X ,robot->Y,DEG(robot->Alpha));
//if(DEG(robot->Alpha)>= 90)
//sleep(1000);


    //Set encoder values
    //--KL Added encoder factor
    if ((abs(robot->Motor[i].Pos - (int)rint(robot->Motor[i].TargetPos)) <=
        (int)(rint(robot->Motor[i].Value)*ENCODER_FACTOR)) &&
         (robot->Motor[i].mode == POSITION_MODE))
      robot->Motor[i].Pos = robot->Motor[i].TargetPos;
    else
	 	  robot->Motor[i].Pos += (int)(rint(robot->Motor[i].Value)*ENCODER_FACTOR);
	}

  //Set simulated robot's position in the world
...
void MessageRobotDeal(struct Context *context, char *q, char *a)
{
  ...
  switch (q[0]) {
    case 'A':
      //fprintf(stderr,"Warning: serial command A (Configure) not implemented");
      strcpy(a,"a\n");
      break;
    case 'B':
      //fprintf(stderr, "Warning: serial command B (Read Software version) not implemented");
      strcpy(a,"b,0\n");
      break;
    case 'C':
      if (sscanf(q,"C,%d,%d",&lm,&rm)==2)
      {
    ...
    case 'D':
      ...
      break;
    case 'E':
      sprintf(a,"e,%d,%d\n",(int)robot->Motor[1].Value,(int)robot->Motor[0].Value);
      break;
    case 'G':
      if (sscanf(q,"G,%d,%d",&lm,&rm)==2)
      {
        robot->Motor[1].Pos=lm;
        robot->Motor[0].Pos=rm;

        //--KL Set target pos to new position
        robot->Motor[1].TargetPos=lm;
        robot->Motor[0].TargetPos=rm;
      }
      strcpy(a,"g\n");
      break;
    case 'H':
      sprintf(a,"h,%d,%d\n",robot->Motor[1].Pos,robot->Motor[0].Pos);
break;
    case 'I':
      sprintf(a,"i,%lf,%lf,%lf\n",robot->X,robot->Y,robot->Alpha);
      break;
    case 'K':
      if(robot->Motor[1].mode == SPEED_MODE)
      ...
      sprintf(a,"k,%d,%d,%d,%d,%d,%d\n",
        ...
        break;
    case 'L':
      //fprintf(stderr,"Warning: serial command L (LED set) not implemented");
      strcpy(a,"l\n");
      break;
    case 'N':
    ...
    case 'O':
      ...
      break;
    default:
      if (strcmp(q, "restart") == 0)
      {
        robot->Motor[1].Pos=lm;
        robot->Motor[0].Pos=rm;
        robot->Motor[1].TargetPos=lm;
        robot->Motor[0].TargetPos=rm;
        strcpy(a, "RESTART COMPLETE");
      }
      else
      {
        strcpy(a,"unrecognized command: ");
        strcat(a,q);
        strcat(a,"\n");
      }
  }
...
void OpenKheperaSerial(struct Context *context)
{
  ...
  if (khepera_fd > 0)
  {
    ...
    if (khep_talk(khepera_fd,"A,3800,800,100\n",answer,TEXT_BUF_SIZE-1))
     DisplayComment(context,"Ready to run epuck robot");
    else DisplayComment(context,"serial communication error");
