...
void OpenProgram(char file_name[])
{
  ...
  else
  {
    context->World = (struct World *)malloc(sizeof(struct World));
    context->World->Objects = NULL;
    printf("Couldn't find world '%s', creating new world...\n", file_name);
    CreateDefaultWorld(context);
    strncpy(context->World->Name,"new",15);
    //perror("unable to find this world in directory");
    //exit(0);
  }
  ...
  //--KL buttons have been moved around a lot
  but= context->Buttons=CreateButton(NEW_WORLD,"new",x1,y2);
  but= but->Next=CreateButton(NEW_WORLD,"new",x1,y2);
  but= but->Next=CreateButton(LOAD_WORLD,"load",x1+28,y2);
  but= but->Next=CreateButton(SAVE_WORLD,"save",x1+63,y2);
  but= but->Next=CreateButton(REDRAW_WORLD,"redraw",x1+98,y2);
	
  but= but->Next=CreateButton(SET_ROBOT,"set robot",x3,y1);
  but= but->Next=CreateButton(SET_ANGLE,"set angle",x3,y2);

  but= but->Next=CreateButton(ADD_OBJECT,"add",x4,y1);
  but= but->Next=CreateButton(TURN_OBJECT,"turn",x4+30,y1);
  //but= but->Next=CreateButton(REMOVE_OBJECT,"remove",x4,y2);
  but= but->Next=CreateButton(OBJECT_PLUS,"+",x4+66,y1);
  but= but->Next=CreateButton(OBJECT_MINUS,"-",x4+66,y2);

  but= but->Next=CreateButton(RUN_ROBOT,"run",x2,23);
  //but= but->Next=CreateButton(RUN_ROBOT_NO_GUI,"run (no gui)",x2, y2);
  but= but->Next=CreateButton(STEP_ROBOT,"step",x2+28, y2);
  //but= but->Next=CreateButton(TEST,"test",x2, y2);
  but= but->Next=CreateButton(RESET_ROBOT,"reset",x2+63, y2);
  but= but->Next=CreateButton(QUIT,"quit",x2+105,y2);
...
int main(int argc,char *argv[]) {
  ...
  //long int          i,j;
  int               angle;
  u_char            initButtonValue = 255;  //--KL Used to be -1
  FILE              *ffile;
  char              *shm_in, *shm_out;
  ...
  if((shm_in = (char*)shmat(shmid_in, NULL, 0)) == ((char *) -1)) {
    perror("shmat");
    exit(1);
  }

  //--KL added explicit cast 
  if((shm_out = (char*)shmat(shmid_out, NULL, 0)) == ((char *) -1)) {
    perror("shmat");
    exit(1);
  }
	...
	while(TRUE) {
	  ...
	  switch(button->Value)
	      ...
	      if (name[0]!='\0')
		{
		  ...
		  strcpy(text,name);
		  strcat(name,".world");
		  strcpy(file_name,""); //--KL removed worlds/ prefix
		  ...
		  else sprintf(text,"unable to find '%s' in directory",name);
		  DisplayComment(context,text);
		}
	    ...
	    case TURN_OBJECT:
	      world->ObjectAlpha[world->ObjectType] += 90; //--KL turn by 90 degree increments
        ...
        //--KL -> redraw world in case robot was moved suddenly
	      //DrawWorld(context);
	    ...
	    case RUN_ROBOT:
	      CancelCursor();
	      DisplayComment(context,"running simulated epuck");
		...
		usleep(TIME_DELAY);
	      ...
	      RunRobotStop(robot);
	      DisplayComment(context,"simulated epuck stopped");
	      PointerCursor();
	      break;
	      
	    case RESET_ROBOT:
	      ResetRobot(robot); //--KL added this
	      InitRobot(context);
	      DrawRobotIRSensors(robot);
	      DrawRobotEffectors(robot);
	      DrawWorld(context); //--KL added this
	      DisplayComment(context,"epuck reset");
	      break;
		...
		{
		  DisplayComment(context,"testing real epuck");
		  while (UnpressButton(context,button,robot)==FALSE)
		    {
		...
		{
		  DisplayComment(context,"testing simulated epuck");
		  while (UnpressButton(context,button,robot)==FALSE)
		    {
		      InitSensors(context);
		      DrawRobotIRSensors(robot);
