...
void DrawIntValue(long int  value,short int x,short int y)
{
  ...
  if (value==-1) sprintf(t,"?"); else sprintf(t,"%ld",value);
  DrawText(x,y,t);
}
...
void DrawRobotIRSensors(struct Robot *robot)
{
  ...
  ClearValC(4,ROBOT_BASE_X+51,ROBOT_BASE_Y-44);
  ClearValC(4,ROBOT_BASE_X+70,ROBOT_BASE_Y);
  ClearValC(4,ROBOT_BASE_X-70,ROBOT_BASE_Y);
...
void DrawRobotEffectors(struct Robot *robot)
{
  ...
  DrawMotorActivity((short int)(ROBOT_BASE_X+ROBOT_DIAMETER*ROBOT_SCALE/3.6),
                    (short int)(robot->Motor[0].Value));
  DrawMotorActivity((short int)(ROBOT_BASE_X-ROBOT_DIAMETER*ROBOT_SCALE/3.6),
                    (short int)(robot->Motor[1].Value));
  Color(LIGHT_GREY);
  ...
  {
    Color(BLACK);
    sprintf(t,"%d",(int)robot->Motor[0].Value);
    DrawText(ROBOT_BASE_X+30-4*strlen(t),ROBOT_BASE_Y+24,t);
    sprintf(t,"%d",(int)robot->Motor[1].Value);
    DrawText(ROBOT_BASE_X-30-4*strlen(t),ROBOT_BASE_Y+24,t);
  }
}
...
void DrawInfoAbout(struct Context *context)
{
  switch(context->InfoAbout)
  {
    case 0: Color(BLUE);
            ...
            Color(YELLOW);
            DrawText(WINDOW_W*3/4-4*17,210,"Epuck Simluator");
            Color(BLACK);
            DrawText(630,270,
                  "This program is a simulator for Epuck robot featuring:");
            DrawText(630,290,"- A simulated Epuck robot");
            DrawText(630,305,"- A world editor");
            DrawText(630,320,"- A graphical user interface");
            DrawText(630,335,"- C programming facilities");
            DrawText(630,365,
            "This software is provided for the Official Epuck Contest");
            DrawText(630,380,
            ...
            DrawText(630,425,"http://alto.unice.fr/~om/Epuck-contest.html");
            DrawText(630,460,
    ...
    case 1: Color(BLUE);
            ...
            Color(YELLOW);
            DrawText(855-4*21,210,"Epuck: presentation");
            Color(BLACK);
            DrawText(680,250,
            "Epuck  is a  real  miniature  mobile  robot");
            DrawText(680,270,
            ...
            DrawText(736,490,
            "http://lamiwww.epfl.ch/Epuck/");
            break;
    case 2: Color(BLUE);
            DrawRectangle(779,198,149,16);
            Color(YELLOW);
            DrawText(855-4*18,210,"Epuck: simulator");
            Color(BLACK);
            DrawText(680,270,
            "The  simulator  includes  all the features of");
            DrawText(680,290,
            "the basic module  of Epuck  (IR sensors and");
            DrawText(680,310,
            ...
            DrawText(680,430,
            "The simulator is provided for Epuck Contest");
            DrawText(680,470,
            "More can be found on the Internet web at URL:");
            DrawText(692,490,
            "http://alto.unice.fr/~om/Epuck-contest.html");
            break;
  }
}
...
void OpenGraphics(struct Context *context)
{
  ...
  XChangeProperty(display,window,XA_WM_NAME,XA_STRING,8,PropModeReplace,
                  (unsigned char *)
                 "Epuck Simulator (adopted Khepra simulator 2.0 by O. MICHEL",60);
  XChangeProperty(display,window,XA_WM_ICON_NAME,XA_STRING,8,PropModeReplace,
                  (unsigned char *)"Epuck Simulator",17);
